...
final getSlackWebhookUrlUseCaseProvider = Provider<GetSlackWebhookUrlUseCase>(
  (ref) => _GetSlackWebhookUrlUseCaseImpl(
    ref.read(chromeApiProvider),
  ),
);

abstract class GetSlackWebhookUrlUseCase {
  Future<SlackWebhookUrl?> call();
}

class _GetSlackWebhookUrlUseCaseImpl extends GetSlackWebhookUrlUseCase {
  _GetSlackWebhookUrlUseCaseImpl(this._chromeApi);

  final ChromeApi _chromeApi;

  @override
  Future<SlackWebhookUrl?> call() async {
    ...
    if (paths != null) {
      ...
      return SlackWebhookUrl(value: 'https://hooks.slack.com/services/$path1/$path2/$path3');
    } else {
      return null;
    }
  }
